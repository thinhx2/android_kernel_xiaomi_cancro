...
static inline u64 get_cpu_idle_time_x(unsigned int cpu, u64 *wall)
{
...
static void cpufreq_smartmax_timer(struct smartmax_info_s *this_smartmax) {
	...
	for_each_cpu(j, policy->cpus)
	{
		...
		cur_idle_time = get_cpu_idle_time_x(j, &cur_wall_time);
...
static void update_idle_time(bool online) {
	...
	for_each_possible_cpu(j)
	{
		...
		j_this_smartmax->prev_cpu_idle = get_cpu_idle_time_x(j,
...
static int cpufreq_smartmax_boost_task(void *data) {
	...
	while (1) {
        ...
        this_smartmax->prev_cpu_idle = get_cpu_idle_time_x(0,
		...
		for_each_online_cpu(cpu){
			...
			if (policy->cur < cur_boost_freq) {
				...
				this_smartmax->prev_cpu_idle = get_cpu_idle_time_x(cpu, &this_smartmax->prev_cpu_wall);
			}
			mutex_unlock(&this_smartmax->timer_mutex);
